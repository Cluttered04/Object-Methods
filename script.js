...
salesByWeek.forEach(sale => {
    //breaks Objects down into arrays
    const saleObjectArrays = Object.entries(sale);
    // console.log(saleObjectArrays);
    //prints agent's name
    console.log(saleObjectArrays[2][1].first_name, saleObjectArrays[2][1].last_name)
    //prints vehicle information
    const vehicleObject = Object.values(Object.entries(saleObjectArrays[0][1]))
    vehicleObject.forEach(vehicle => {
        console.log(vehicle);
    })
    //prints gross profit
    console.log(saleObjectArrays[4])
    })
...
const searchInput = document.querySelector("#searchInput")
    ...
    searchInput.addEventListener('keypress', event => {
      if (event.charCode === 13) {
        ...
        salesByWeek.forEach(sale => {
            const agentInfo = Object.values(Object.entries(sale)[2][1]);
            console.log(agentInfo);
            agentInfo.forEach(singleDetail => {
            if(singleDetail.includes(searchTerm)){
                const printAgent = `<h1>${agentInfo[2]} ${agentInfo[1]}</h1><p></p><p> ${agentInfo[3]}</p>`
                document.querySelector("#results").innerHTML += printAgent
            }})
        });
      }
    })
